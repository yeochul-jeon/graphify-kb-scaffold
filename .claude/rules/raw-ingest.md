...
⚠️ **새 열거값(`unobtainable` 등)·새 필드·승인 체크리스트 0번의 새 갈래를 만들지 않는다.** 보류된 건은 0번에서 **여전히 "필드 없음"** 이고 현행 (a)/(b)/(c) 가 그대로 적용된다. **이것은 부작용이 아니라 의도된 결과다** — 원본을 시점 고정할 수 없으면 저장분이 요약본인지 배제할 수 없고, 그것이 0번이 존재하는 이유다((c)-ⓑ 와 같은 논리).
