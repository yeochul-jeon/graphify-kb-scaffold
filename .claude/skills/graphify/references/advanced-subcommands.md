# Graphify Advanced Subcommands

> **Lazy-loaded reference**. Read this file only when executing one of the subcommands below.
> Core workflow is in `../SKILL.md`.

Subcommands covered:
- `--update` (incremental re-extraction)
- `--cluster-only` (re-cluster existing graph)
- `/graphify query` (BFS / DFS traversal)
...
```bash
scripts/graphify-py.sh -m graphify save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
