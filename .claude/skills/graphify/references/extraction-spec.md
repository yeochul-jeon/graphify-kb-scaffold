...
_origin RULE (every node and edge, NOT hyperedges): emit `"_origin":"semantic"` verbatim on every node and every edge. This declares the extraction tier explicitly. Without it the engine infers the tier from the SHAPE of source_location, and any value matching `^L\d` — a line range like `"L73-82"` — is read as deterministic AST output. A fragment misread that way does not replace the stored semantic node of the same id: both records reach dedup, and the collision is decided by label length, so a freshly extracted longer label is silently discarded in favour of the stale shorter one. Keep source_location `null` for the same reason — a line range is the AST extractor's signature, not yours. Do NOT put _origin on hyperedges; they are semantic-tier by construction.
